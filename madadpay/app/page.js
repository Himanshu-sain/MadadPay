...
const HomePage = () => {
    ...
    const autoDetectLocation = async () => {
      ...
      try {
        ...
        if (permissionStatus.state === 'granted') {
          ...
          setDisplayLocation('Allow location access for better results');
        }
        ...
        setLocationStatus('error');
        setDisplayLocation('Location detection failed');
      }
    };
  ...
  const getLocation = async () => {
    ...
    try {
      ...
      switch(error.code) {
        case error.PERMISSION_DENIED:
          errorMessage = 'Location permission denied. Click below to enable.';
          break;
        case error.TIMEOUT:
      ...
      <div className="bg-indigo-900/80 backdrop-blur-lg p-8 rounded-2xl border border-indigo-700/50 shadow-xl max-w-3xl w-full">
        ...
        {locationStatus === "error" && (
          <motion.div 
            ...
            className="mt-4 p-4 bg-red-900/30 rounded-lg border border-red-800"
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-400">📍 {displayLocation}</p>
        ...
        {locationStatus === "success" && (
          <p className="text-xs text-indigo-300 mt-3 text-center">
            Your location is only used to find nearby exchanges
          </p>
        )}
      </div>
    </div>
